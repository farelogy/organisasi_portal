...
@php
    $typeColors = [
        ...
        'konvensi' => 'bg-purple-100 text-purple-700',
    ];
...
<div class="bg-white rounded-2xl shadow-md border border-gray-100 overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group flex flex-col"
    ...
    <div class="relative h-48 overflow-hidden">
        ...
        <div class="absolute top-3 left-3">
            @if($isPast)
            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-gray-700/80 text-white backdrop-blur-sm">Selesai</span>
            @else
            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-green-500/90 text-white backdrop-blur-sm">
        ...
        <span class="absolute top-3 right-3 inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold {{ $typeColor }} backdrop-blur-sm capitalize">{{ $event->type }}</span>

        <!-- Date block bottom-left -->
        @if($event->event_date)
        <div class="absolute bottom-3 left-3 bg-white rounded-xl px-3 py-1.5 shadow-lg text-center leading-tight">
            <div class="text-2xl font-extrabold text-orange-500 leading-none">{{ $event->event_date->format('d') }}</div>
            <div class="text-xs font-bold text-gray-500 uppercase tracking-wider">{{ $event->event_date->format('M Y') }}</div>
        </div>
        @endif
    </div>
    ...
    <div class="flex flex-col flex-1 p-5">
        ...
        <div class="flex items-center gap-1.5 mb-3">
            <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-orange-500 text-white">
        ...
        <div class="flex flex-wrap gap-1.5 mb-3">
            @if($event->category)
            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-600 capitalize">{{ $event->category }}</span>
            @endif
            @if($event->sub_category)
            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-orange-50 text-orange-600">{{ $event->sub_category }}</span>
            @endif
        </div>

        <!-- Title -->
        <h3 class="text-base font-bold text-gray-900 mb-2 group-hover:text-orange-600 transition-colors line-clamp-2 leading-snug">{{ $event->title }}</h3>
        ...
        <div class="flex flex-col gap-1.5 text-xs text-gray-500 border-t border-gray-100 pt-3 mb-4">
            ...
            @if($event->event_date)
            <span class="flex items-center gap-1.5">
                <svg class="w-3.5 h-3.5 text-orange-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
