...
<section class="relative py-24 bg-gradient-to-br from-slate-900 via-orange-900 to-slate-800 overflow-hidden">
    <div class="absolute inset-0 opacity-10" style="background-image: radial-gradient(circle at 25% 25%, rgba(255,107,53,0.3) 0%, transparent 50%);"></div>
    <div class="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8" data-aos="fade-up">
        <a href="{{ route('event.index') }}" class="inline-flex items-center text-orange-300 hover:text-orange-200 text-sm font-medium mb-6 transition-colors">
        ...
        <div class="flex flex-wrap gap-2 mb-4">
            ...
            @if($event->category)
            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-white/20 text-white capitalize">{{ $event->category }}</span>
            @endif
            @if($event->sub_category)
            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-orange-500/30 text-orange-200">{{ $event->sub_category }}</span>
            @endif
            @if($isPast)
            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-gray-500/30 text-gray-300">Selesai</span>
            @else
            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-green-500/30 text-green-300">
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-1">
                <div class="bg-gradient-to-br from-orange-50 to-orange-100 rounded-2xl p-6 border border-orange-200 sticky top-24">
                    <h3 class="text-lg font-bold text-gray-900 mb-4">Informasi Event</h3>
                    <div class="space-y-4">
                        @if($event->event_date)
                        <div>
                            <div class="text-xs font-semibold text-orange-600 uppercase tracking-wide mb-1">Tanggal</div>
                        ...
                        @if($event->location)
                        <div>
                            <div class="text-xs font-semibold text-orange-600 uppercase tracking-wide mb-1">Lokasi</div>
                            <div class="text-gray-800 font-medium">{{ $event->location }}</div>
                        </div>
                        @endif
                        @if($event->type)
                        <div>
                            <div class="text-xs font-semibold text-orange-600 uppercase tracking-wide mb-1">Tipe Kegiatan</div>
                            <span class="inline-block px-3 py-1 rounded-full text-xs font-semibold {{ $typeColor }} capitalize">{{ $event->type }}</span>
                        </div>
                        @endif
                        @if($event->category)
                        <div>
                            <div class="text-xs font-semibold text-orange-600 uppercase tracking-wide mb-1">Kategori</div>
                    ...
                    <div class="mt-6">
                        <a href="{{ $event->link }}" target="_blank" class="block w-full text-center px-6 py-3 bg-gradient-to-r from-orange-500 to-orange-600 text-white rounded-xl font-semibold hover:from-orange-600 hover:to-orange-700 transition-all duration-300 shadow-md">
                            Daftar Sekarang
                    ...
                    @elseif($isPast)
                    <div class="mt-6 p-3 bg-gray-100 rounded-xl text-center text-gray-500 text-sm font-medium">Pendaftaran telah ditutup</div>
                    @endif

                    <div class="mt-4">
                        <a href="{{ route('event.index') }}" class="block w-full text-center px-6 py-3 bg-white text-orange-600 rounded-xl font-semibold hover:bg-orange-50 transition-all duration-300 border border-orange-200">
                            Lihat Event Lainnya
                        </a>
                    </div>
                </div>
