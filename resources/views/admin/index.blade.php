...
<aside class="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <button onclick="showTab('users')" id="nav-users" class="nav-item">
            ...
            <span class="nav-count">{{ $users->count() ?? 0 }}</span>
        </button>
    </nav>
    <div class="sidebar-footer" style="display:flex;flex-direction:column;gap:8px;">
        ...
        <form action="{{ route('logout') }}" method="POST" style="width:100%;">
            @csrf
            <button type="submit" class="btn-outline" style="width:100%; justify-content:center; color:#f87171; border-color:rgba(248,113,113,0.3);">
    ...
    <div class="content-area">
        ...
        <div class="stats-grid">
            <div class="stat-card">
                ...
                <div class="stat-num">{{ $beritas->total() }}</div>
            ...
            <div class="stat-card">
                ...
                <div class="stat-num">{{ $artikels->count() }}</div>
                <div class="stat-label">Total Artikel</div>
            </div>
            <div class="stat-card">
                <div class="stat-icon" style="background:#fdf4ff;">🖼️</div>
                <div class="stat-num">{{ $galleries->count() }}</div>
        ...
        <div id="tab-heroes" class="tab-panel active">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="item-row">
                                @if($hero->image)
                                <img src="{{ $hero->image }}" class="item-thumb" alt="{{ $hero->title }}">
                                @else
                                ...
                                <div class="item-info">
                                    ...
                                    <div style="margin-top:6px;">
                                        <span class="{{ $hero->is_active ? 'badge-active' : 'badge-inactive' }}">
                                            {{ $hero->is_active ? 'Aktif' : 'Non-Aktif' }}
                                        </span>
                                    </div>
                                </div>
                                <button onclick="showForm(event, 'hero', {{ $hero->id }})" class="edit-btn" 
                        ...
                        <div class="empty-state">
                            ...
                            <div class="empty-desc">Tambahkan hero section untuk ditampilkan di halaman utama</div>
                        </div>
                        @endif
                    </div>
                </div>
                ...
                <div id="form-panel-hero" class="card" style="display:none;">
                    ...
                    <div class="card-body">
                        <form action="{{ route('admin.heroes.store') }}" method="POST" enctype="multipart/form-data" id="form-hero">
                            @csrf
                            <input type="hidden" name="_method" value="POST" id="hero-method">
                            <div class="form-group">
                                <label class="form-label">Title</label>
                                <input type="text" name="title" class="form-input" required placeholder="Judul hero section">
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="button_text" class="form-input" placeholder="Teks tombol">
                            </div>
                            <div class="form-group">
                                <label class="form-label">Button Link</label>
                                <input type="text" name="button_link" class="form-input" placeholder="URL tombol">
                            </div>
                            <div style="display:flex;align-items:center;gap:8px;">
                                <input type="hidden" name="is_active" value="0">
                                <input type="checkbox" name="is_active" value="1" id="hero-active" style="width:16px;height:16px;accent-color:#f97316;" checked>
                                <label for="hero-active" class="form-label" style="margin:0;">Aktif</label>
                            </div>
                            <button type="submit" class="btn-orange" style="width:100%;justify-content:center;margin-top:16px;">Simpan</button>
                        </form>
                    </div>
                </div>
            </div>
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="item-row">
                                ...
                                <div class="item-info">
                                    ...
                                    <div class="item-meta">{{ Str::limit($layanan->description, 70) }}</div>
                                    <div style="margin-top:6px; display:flex; gap:8px; align-items:center;">
                                        <span class="{{ $layanan->is_active ? 'badge-active' : 'badge-inactive' }}">{{ $layanan->is_active ? 'Aktif' : 'Non-Aktif' }}</span>
                                        <span style="font-size:11px;color:#d1d5db;">Urutan: {{ $layanan->order }}</span>
                                    </div>
                                </div>
                                <button onclick="showForm(event, 'layanan', {{ $layanan->id }})" class="edit-btn">Edit</button>
                            </div>
                            @endforeach
                        </div>
                        @else
                ...
                <div id="form-panel-layanan" class="card" style="display:none;">
                    ...
                    <div class="card-body">
                        <form action="{{ route('admin.layanans.store') }}" method="POST" id="form-layanan">
                            ...
                            <div style="display:flex;align-items:center;gap:8px;">
                                <input type="hidden" name="is_active" value="0">
                                <input type="checkbox" name="is_active" value="1" id="layanan-active" style="width:16px;height:16px;accent-color:#f97316;" checked>
        ...
        <div id="tab-beritas" class="tab-panel">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="item-row">
                                @if($berita->image)
                                <img src="{{ $berita->image }}" class="item-thumb" alt="{{ $berita->title }}">
                                @else
                                <div class="item-thumb-placeholder">📰</div>
                                @endif
                                <div class="item-info">
                                    <div class="item-title">{{ $berita->title }}</div>
                                    <div class="item-meta">{{ Str::limit($berita->excerpt ?? '', 70) }}</div>
                                    <div style="margin-top:6px;">
                                        <span class="{{ $berita->is_active ? 'badge-active' : 'badge-inactive' }}">{{ $berita->is_active ? 'Aktif' : 'Non-Aktif' }}</span>
                                    </div>
                                </div>
                                <button onclick="showForm(event, 'berita', {{ $berita->id }})" class="edit-btn"
                ...
                <div id="form-panel-berita" class="card" style="display:none;">
                    ...
                    <div class="card-body">
                        <form action="{{ route('admin.beritas.store') }}" method="POST" enctype="multipart/form-data" id="form-berita">
                            @csrf
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="content" id="berita-content" class="form-input form-textarea tinymce-editor" rows="10" required></textarea>
                            </div>
                            <div class="form-group">
                                <label class="form-label">Gambar</label>
                                <input type="file" name="image" accept="image/*" class="form-input">
                            ...
                            <div style="display:flex;align-items:center;gap:8px;">
                                <input type="hidden" name="is_active" value="0">
                                <input type="checkbox" name="is_active" value="1" id="berita-active" style="width:16px;height:16px;accent-color:#f97316;" checked>
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:24px;">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{ route('admin.sejarahs.store') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @if($errors->any())
                            <div style="background:#fef2f2;border:1px solid #fecaca;border-radius:10px;padding:12px 16px;margin-bottom:16px;font-size:13px;color:#b91c1c;">
                                @foreach($errors->all() as $e)<div>• {{ $e }}</div>@endforeach
                            </div>
                            @endif
                            <div class="form-group">
                                <label class="form-label">Judul</label>
                                <input type="text" name="title" value="{{ $sejarah->title ?? '' }}" class="form-input" required placeholder="Judul konten sejarah">
                            </div>
                            <div class="form-group">
                                ...
                                <p style="font-size:12px;color:#9ca3af;margin-top:6px;">
                                    Saat ini: <a href="{{ $sejarah->image }}" target="_blank" style="color:#f97316;font-weight:600;">Lihat Gambar</a>
                                </p>
                                @endif
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="content" id="sejarah-editor" class="form-input form-textarea tinymce-editor" required placeholder="Tuliskan sejarah PII di sini...">{{ $sejarah->content ?? '' }}</textarea>
                            </div>
                            <div style="display:flex;align-items:center;gap:8px;margin-bottom:18px;">
                                <input type="hidden" name="is_active" value="0">
                                <input type="checkbox" name="is_active" value="1" id="is_active_sejarah" style="width:16px;height:16px;accent-color:#f97316;" {{ ($sejarah->is_active ?? true) ? 'checked' : '' }}>
                                <label for="is_active_sejarah" class="form-label" style="margin:0;">Tampilkan di website</label>
                            </div>
                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        <span class="badge-active">{{ $ketuaUmums->count() }} data</span>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('admin.ketuaUmums.store') }}" method="POST" enctype="multipart/form-data" id="ketua-umum-form" style="padding-bottom:16px;border-bottom:1px solid #f3f4f6;margin-bottom:16px;">
                            @csrf
                            ...
                            <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="name" id="ketua-umum-name" class="form-input" required placeholder="Nama lengkap">
                                </div>
                                <div class="form-group">
                                    <label class="form-label">Periode</label>
                                    <input type="text" name="period" id="ketua-umum-period" class="form-input" placeholder="cth: 2020–2024">
                                </div>
                            </div>
                            <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                                <div class="form-group">
                                    <label class="form-label">Foto</label>
                                    <input type="file" name="image" id="ketua-umum-image" accept="image/*" class="form-input">
                                </div>
                                <div class="form-group">
                                    <label class="form-label">Urutan</label>
                                    <input type="number" name="order" id="ketua-umum-order" value="0" class="form-input">
                                </div>
                            </div>
                            <div style="display:flex;align-items:center;gap:8px;margin-bottom:14px;">
                                <input type="hidden" name="is_active" value="0">
                                <input type="checkbox" name="is_active" value="1" id="ketua-umum-is_active" style="width:16px;height:16px;accent-color:#f97316;" checked>
                                <label for="ketua-umum-is_active" class="form-label" style="margin:0;">Aktifkan</label>
                            </div>
                            ...
                            <button type="button" id="ketua-umum-cancel" onclick="cancelEditKetuaUmum()" class="btn-cancel" style="width:100%;justify-content:center;margin-top:8px;display:none;">Batal</button>
                        </form>
                        ...
                        <div style="max-height:260px;overflow-y:auto;">
                            ...
                            <div class="item-row">
                                @if($k->image)
                                <img src="{{ $k->image }}" class="item-thumb" alt="{{ $k->name }}" style="width:44px;height:44px;border-radius:50%;">
                                @else
                                <div class="item-thumb-placeholder" style="width:44px;height:44px;border-radius:50%;font-size:18px;">👤</div>
                                @endif
                                <div class="item-info">
                                    <div class="item-title" style="font-size:13px;">{{ $k->name }}</div>
                                    <div class="item-meta">{{ $k->period }} · Urutan: {{ $k->order }}</div>
                                </div>
                                <div style="display:flex;gap:6px;align-items:center;">
                                    <button type="button" onclick="editKetuaUmum(event, {{ $k->id }})" class="edit-btn"
                                        data-name="{{ $k->name }}"
                                        data-period="{{ $k->period }}"
                                        data-order="{{ $k->order }}"
                                        data-is_active="{{ $k->is_active ? 'true' : 'false' }}">✏️</button>
                                    <form action="{{ route('admin.ketuaUmums.delete', $k->id) }}" method="POST" style="display:inline;">
                                        @method('DELETE')
                                        @csrf
                                        <button type="submit" style="background:none;border:none;cursor:pointer;color:#f87171;font-size:16px;" onclick="return confirm('Hapus?')">🗑️</button>
                                    </form>
                                </div>
                            </div>
                            @endforeach
        ...
        <div id="tab-sekilas" class="tab-panel">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="item-row">
                                @if($s->image)<img src="{{ $s->image }}" class="item-thumb" alt="{{ $s->title }}">
                                @else<div class="item-thumb-placeholder">👁️</div>@endif
                                <div class="item-info">
                                    <div class="item-title">{{ $s->title }}</div>
                                    <div class="item-meta">{{ Str::limit($s->content, 70) }}</div>
                                    <div style="margin-top:6px;"><span class="{{ $s->is_active ? 'badge-active' : 'badge-inactive' }}">{{ $s->is_active ? 'Aktif' : 'Non-Aktif' }}</span></div>
                                </div>
                                <button onclick="showForm(event, 'sekilas', {{ $s->id }})" class="edit-btn">Edit</button>
                            </div>
                            @endforeach
                        </div>
                        @else
                ...
                <div id="form-panel-sekilas" class="card" style="display:none;">
                    ...
                    <div class="card-body">
                        <form action="{{ route('admin.sekilas.store') }}" method="POST" enctype="multipart/form-data" id="form-sekilas">
                            @csrf
                            ...
                            <div style="display:flex;align-items:center;gap:8px;">
                                <input type="hidden" name="is_active" value="0">
                                <input type="checkbox" name="is_active" value="1" id="sekilas-active" style="width:16px;height:16px;accent-color:#f97316;" checked>
        ...
        <div id="tab-strukturs" class="tab-panel">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="item-row">
                                @if($s->photo)<img src="{{ $s->photo }}" class="item-thumb" alt="{{ $s->name }}" style="border-radius:50%;">
                                @else<div class="item-thumb-placeholder" style="border-radius:50%;">👤</div>@endif
                                <div class="item-info">
                                    <div class="item-title">{{ $s->name }}</div>
                                    <div class="item-meta" style="color:#f97316;font-weight:500;">{{ $s->position }}</div>
                                    <div style="margin-top:6px;display:flex;gap:8px;align-items:center;">
                                        <span class="{{ $s->is_active ? 'badge-active' : 'badge-inactive' }}">{{ $s->is_active ? 'Aktif' : 'Non-Aktif' }}</span>
                                        <span style="font-size:11px;color:#d1d5db;">Order: {{ $s->order }}</span>
                                    </div>
                                </div>
                                <button onclick="showForm(event, 'struktur', {{ $s->id }})" class="edit-btn">Edit</button>
                            </div>
                            @endforeach
                        </div>
                        @else
                ...
                <div id="form-panel-struktur" class="card" style="display:none;">
                    ...
                    <div class="card-body">
                        <form action="{{ route('admin.strukturs.store') }}" method="POST" enctype="multipart/form-data" id="form-struktur">
                            @csrf
                            ...
                            <div style="display:flex;align-items:center;gap:8px;">
                                <input type="hidden" name="is_active" value="0">
                                <input type="checkbox" name="is_active" value="1" id="struktur-active" style="width:16px;height:16px;accent-color:#f97316;" checked>
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="item-row">
                                ...
                                <div class="item-info">
                                    ...
                                    <div class="item-meta">{{ $k->phone }} · {{ $k->email }}</div>
                                    <div style="margin-top:6px;"><span class="{{ $k->is_active ? 'badge-active' : 'badge-inactive' }}">{{ $k->is_active ? 'Aktif' : 'Non-Aktif' }}</span></div>
                                </div>
                                <button onclick="showForm(event, 'kontak', {{ $k->id }})" class="edit-btn">Edit</button>
                            </div>
                            @endforeach
                        </div>
                        @else
                ...
                <div id="form-panel-kontak" class="card" style="display:none;">
                    ...
                    <div class="card-body">
                        <form action="{{ route('admin.kontaks.store') }}" method="POST" id="form-kontak">
                            ...
                            <div style="display:flex;align-items:center;gap:8px;">
                                <input type="hidden" name="is_active" value="0">
                                <input type="checkbox" name="is_active" value="1" id="kontak-active" style="width:16px;height:16px;accent-color:#f97316;" checked>
        ...
        <div id="tab-events" class="tab-panel">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="space-y-4">
                            @foreach($events as $e)
                            <div class="item-row">
                                @if($e->image)<img src="{{ $e->image }}" class="item-thumb" alt="{{ $e->title }}">
                                @else<div class="item-thumb-placeholder">📅</div>@endif
                                <div class="item-info">
                                    <div class="item-title">{{ $e->title }}</div>
                                    <div class="item-meta" style="color:#f97316;font-weight:500;">{{ $e->type }} · {{ $e->event_date->format('d M Y') }}</div>
                                    <div class="item-meta">📍 {{ $e->location }}</div>
                                    <div style="margin-top:6px;"><span class="{{ $e->is_active ? 'badge-active' : 'badge-inactive' }}">{{ $e->is_active ? 'Aktif' : 'Non-Aktif' }}</span></div>
                                </div>
                                <button onclick="showForm(event, 'event', {{ $e->id }})" class="edit-btn">Edit</button>
                            </div>
                            @endforeach
                        </div>
                        @else
                ...
                <div id="form-panel-event" class="card" style="display:none;">
                    ...
                    <div class="card-body">
                        <form action="{{ route('admin.events.store') }}" method="POST" enctype="multipart/form-data" id="form-event">
                            @csrf
                            ...
                            <div style="display:flex;align-items:center;gap:8px;">
                                <input type="hidden" name="is_active" value="0">
                                <input type="checkbox" name="is_active" value="1" id="event-active" style="width:16px;height:16px;accent-color:#f97316;" checked>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="item-row">
                                @if($a->image)<img src="{{ $a->image }}" class="item-thumb" alt="{{ $a->title }}">
                                @else<div class="item-thumb-placeholder">📝</div>@endif
                                <div class="item-info">
                                    <div class="item-title">{{ $a->title }}</div>
                                    <div class="item-meta" style="color:#f97316;font-weight:500;">{{ str_replace('_',' ',$a->category) }} · {{ $a->author }}</div>
                                    <div class="item-meta">{{ Str::limit($a->excerpt, 60) }}</div>
                                    <div style="margin-top:6px;display:flex;gap:8px;align-items:center;">
                                        <span class="{{ $a->is_active ? 'badge-active' : 'badge-inactive' }}">{{ $a->is_active ? 'Aktif' : 'Non-Aktif' }}</span>
                                        @if($a->published_at)<span style="font-size:11px;color:#d1d5db;">{{ $a->published_at->format('d M Y') }}</span>@endif
                                    </div>
                                </div>
                                <button onclick="showForm(event, 'artikel', {{ $a->id }})" class="edit-btn">Edit</button>
                            </div>
                            @endforeach
                        </div>
                        @else
                ...
                <div id="form-panel-artikel" class="card" style="display:none;">
                    ...
                    <div class="card-body">
                        <form action="{{ route('admin.artikels.store') }}" method="POST" enctype="multipart/form-data" id="form-artikel">
                            ...
                            <div style="display:flex;align-items:center;gap:8px;">
                                <input type="hidden" name="is_active" value="0">
                                <input type="checkbox" name="is_active" value="1" id="artikel-active" style="width:16px;height:16px;accent-color:#f97316;" checked>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="item-row">
                                @if($g->image)<img src="{{ $g->image }}" class="item-thumb" alt="{{ $g->title }}">
                                @else<div class="item-thumb-placeholder">🖼️</div>@endif
                                <div class="item-info">
                                    <div class="item-title">{{ $g->title }}</div>
                                    @if($g->category)<div class="item-meta" style="color:#f97316;font-weight:500;">{{ $g->category }}</div>@endif
                                    <div class="item-meta">{{ Str::limit($g->description, 60) }}</div>
                                    <div style="margin-top:6px;display:flex;gap:8px;align-items:center;">
                                        <span class="{{ $g->is_active ? 'badge-active' : 'badge-inactive' }}">{{ $g->is_active ? 'Aktif' : 'Non-Aktif' }}</span>
                                        <span style="font-size:11px;color:#d1d5db;">Order: {{ $g->order }}</span>
                                    </div>
                                </div>
                                <button onclick="showForm(event, 'gallery', {{ $g->id }})" class="edit-btn"
                                    data-title="{{ $g->title }}"
                ...
                <div id="form-panel-gallery" class="card" style="display:none;">
                    ...
                    <div class="card-body">
                        <form action="{{ route('admin.galleries.store') }}" method="POST" enctype="multipart/form-data" id="form-gallery">
                            ...
                            <div class="form-group">
                                <label class="form-label">Title</label>
                                <input type="text" name="title" id="gallery-title" class="form-input" required>
                            </div>
                            <div class="form-group">
                                <label class="form-label">Category</label>
                                <input type="text" name="category" id="gallery-category" class="form-input">
                            </div>
                            <div class="form-group">
                                <label class="form-label">Description</label>
                            ...
                            <div class="form-group">
                                <label class="form-label">Gambar</label>
                                <input type="file" name="image" id="gallery-image" accept="image/*" class="form-input">
                                <p style="font-size:11px;color:#9ca3af;margin-top:4px;">Kosongkan jika ingin mempertahankan gambar saat ini (mode edit).</p>
                            </div>
                            <div class="form-group">
                                <label class="form-label">Urutan</label>
                                <input type="number" name="order" id="gallery-order" value="0" class="form-input">
                            </div>
                            <div style="display:flex;align-items:center;gap:8px;">
                                <input type="hidden" name="is_active" value="0">
                                <input type="checkbox" name="is_active" value="1" id="gallery-active" style="width:16px;height:16px;accent-color:#f97316;" checked>
        ...
        <div id="tab-kemitraans" class="tab-panel">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="item-row">
                                @if($km->logo)<img src="{{ $km->logo }}" class="item-thumb" alt="{{ $km->name }}" style="object-fit:contain;background:#f9fafb;padding:4px;">
                                @else<div class="item-thumb-placeholder">🤝</div>@endif
                                <div class="item-info">
                                    <div class="item-title">{{ $km->name }}</div>
                                    <div class="item-meta" style="color:#f97316;font-weight:500;">{{ str_replace('_',' ',$km->type) }}</div>
                                    <div class="item-meta">{{ Str::limit($km->description, 60) }}</div>
                                    <div style="margin-top:6px;display:flex;gap:8px;align-items:center;">
                                        <span class="{{ $km->is_active ? 'badge-active' : 'badge-inactive' }}">{{ $km->is_active ? 'Aktif' : 'Non-Aktif' }}</span>
                                        <span style="font-size:11px;color:#d1d5db;">Order: {{ $km->order }}</span>
                                    </div>
                                </div>
                                <button onclick="showForm(event, 'kemitraan', {{ $km->id }})" class="edit-btn">Edit</button>
                            </div>
                            @endforeach
                        </div>
                        @else
                ...
                <div id="form-panel-kemitraan" class="card" style="display:none;">
                    ...
                    <div class="card-body">
                        <form action="{{ route('admin.kemitraans.store') }}" method="POST" enctype="multipart/form-data" id="form-kemitraan">
                            @csrf
                            ...
                            <div style="display:flex;align-items:center;gap:8px;">
                                <input type="hidden" name="is_active" value="0">
                                <input type="checkbox" name="is_active" value="1" id="kemitraan-active" style="width:16px;height:16px;accent-color:#f97316;" checked>
                                <label for="kemitraan-active" class="form-label" style="margin:0;">Aktif</label>
                            </div>
                            <button type="submit" class="btn-orange" style="width:100%;justify-content:center;margin-top:16px;">Simpan</button>
                        </form>
                    </div>
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="item-row">
                                ...
                                <div class="item-info">
                                    ...
                                    <div class="item-meta">{{ $u->email }} · {{ $u->role ?? 'user' }}</div>
                                    <div style="margin-top:6px;">
                                        <span class="{{ $u->role === 'admin' ? 'badge-active' : 'badge-inactive' }}">
                                            {{ $u->role === 'admin' ? 'Admin' : 'User' }}
                                        </span>
                                    </div>
                                </div>
                                <button onclick="showForm(event, 'user', {{ $u->id }})" class="edit-btn"
                                    data-name="{{ $u->name }}"
                                    data-email="{{ $u->email }}"
                    ...
                    <div class="card-body">
                        <form action="{{ route('admin.users.store') }}" method="POST" id="form-user">
                            ...
                            <div class="form-group">
                                ...
                                <input type="password" name="password" id="user-password" class="form-input" placeholder="Kosongkan untuk tetap menggunakan password saat ini (mode edit)">
                            </div>
                            <div class="form-group">
...
<script>
    ...
    function editKetuaUmum(event, id) {
        ...
        if (actions) actions.innerHTML = '<button type="submit" class="btn-orange" style="width:100%;justify-content:center;"><svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg> Simpan Perubahan</button>';
    ...
    function cancelEditKetuaUmum() {
        ...
        const actions = document.getElementById('ketua-umum-form-actions');
        if (actions) actions.innerHTML = '<button type="submit" class="btn-orange" style="width:100%;justify-content:center;"><svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg> Tambah Ketua Umum</button>';
    ...
    function showToast(message, type = 'success', duration = 4000) {
        ...
        toast.innerHTML = '<span style="font-size:18px;font-weight:700;">' + icon + '</span><span>' + message + '</span>';
                ...
                img.onload = () => {
                    ...
                    const tryCompress = () => {
                        canvas.toBlob((blob) => {
                            ...
                            console.log(`Compressed: ${(file.size / 1024).toFixed(1)}KB → ${(compressedFile.size / 1024).toFixed(1)}KB (quality: ${currentQuality.toFixed(1)})`);
                ...
                for (const input of fileInputs) {
                    if (input.files.length > 0 && input.files[0].type.startsWith('image/')) {
                        ...
                        if (original.size > 2 * 1024 * 1024) {
                            submitBtn.innerHTML = '<span class="spinner"></span> Mengompres gambar...';
                            try {
                                ...
                                showToast(`Gambar dikompres: ${(original.size / 1024).toFixed(0)}KB → ${(compressed.size / 1024).toFixed(0)}KB`, 'success', 2000);
                            } catch (err) {
