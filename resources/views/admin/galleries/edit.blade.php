                    ...
                    <form action="{{ route('admin.galleries.update', $gallery->id) }}" method="POST"
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500 mt-1">Kosongkan jika ingin mempertahankan gambar saat ini.
                                </p>
                            </div>
                            ...
                            <div>
                                ...
                                <textarea name="description" rows="3"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500">{{ $gallery->description }}</textarea>
                            </div>
                ...
                <div class="bg-white rounded-xl shadow-lg p-6">
                    ...
                    <div class="border-2 border-dashed border-gray-300 rounded-lg p-4 min-h-[400px]">
                        <div class="relative overflow-hidden rounded-2xl shadow-lg">
                            <img id="preview-img"
                                src="{{ Str::startsWith($gallery->image, ['http://', 'https://']) ? $gallery->image : asset($gallery->image) }}"
                                alt="Preview" class="w-full h-64 object-cover">
                            <div
                                ...
                                <span id="preview-category"
                                    class="inline-block bg-orange-500 text-white text-xs px-3 py-1 rounded-full font-semibold mb-2 capitalize w-fit {{ $gallery->category ? '' : 'hidden' }}">{{ $gallery->category }}</span>
                                <h3 id="preview-title" class="text-white font-bold text-lg">{{ $gallery->title }}</h3>
                                <p id="preview-description" class="text-gray-200 text-sm mt-1 line-clamp-2">
